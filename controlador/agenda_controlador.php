...
class Agenda extends Controlador
{
    ...
    public function Establecer_Consultas()
    {
        ...
        foreach ($this->datos["lugar_evento"] as $c) {$ubicaciones[] = $c['nombre_lugar_evento'];}
            ...
            case 'Consulta_Ajax':
                ...
                for ($i = 0; $i < count($this->datos["agenda"]); $i++) {
                    ...
                    $retornar[] = [
                        ...
                        'editar'      => "<button title='Editar este evento' class='btn' style='background:#EEA000;color:white' onclick='editar_evento(`" . json_encode($this->datos["agenda"][$i]) . "`,`" . json_encode($this->datos["agenda"]) . "`)'><em class='fas fa-edit'</em></button>",
                        'eliminar'    => "<button title='Eliminar este evento' class='btn' style='background:#9D2323;color:white' onclick='eliminar_evento(`". $this->datos["agenda"][$i]['id_agenda'] . "`)'><em class='fas fa-trash'></em></button>",
                        'ver'         => "<button class='btn' style='background:#15406D;color:white' title='Ver este evento' onclick='ver_evento(`" . json_encode($this->datos["agenda"][$i]) . "`,`" . $user . "`)'  ><em class='fas fa-eye'></em></button>",
                    ];
                }
