...
class Reportes_Class extends Modelo 
{
   ...
   /*   public function Embarazadas() 
    ...
    { 
        ...
        $tabla = "SELECT DISTINCT p.cedula_persona, f.*, fp.*, p.*, v.*, c.*, vc.*, cv.* FROM familia f, familia_personas fp, personas p, vivienda v, calles c, votantes_centro_votacion vc, centros_votacion cv WHERE  f.id_familia = fp.id_familia AND p.cedula_persona = fp.cedula_persona AND v.id_calle = c.id_calle AND p.estado = 1 AND vc.id_centro_votacion = cv.id_centro_votacion and vc.cedula_votante = fp.cedula_persona ORDER BY `c`.`nombre_calle` ASC";
        $respuesta_arreglo = '';
        try {
            $datos = $this->conexion->prepare($tabla);
            $datos->execute();
    ...
    {
        $tabla = "SELECT p.cedula_persona, p.primer_nombre, p.segundo_nombre , p.primer_apellido, p.segundo_apellido, p.nivel_educativo,c.nombre_calle FROM familia f, familia_personas fp, personas p,vivienda v,calles c WHERE p.estado = 1 AND  f.id_familia = fp.id_familia and p.cedula_persona = fp.cedula_persona and v.id_calle = c.id_calle  ORDER BY `c`.`nombre_calle` ASC";
        $respuesta_arreglo = '';
        try {
            $datos = $this->conexion->prepare($tabla);
            $datos->execute();
    ...
    public function Comites_Personas()
    {
        ...
        $tabla = "SELECT * FROM familia f, familia_personas fp, personas p,vivienda v,calles c,comite_persona cc,comite co WHERE  f.id_familia = fp.id_familia and p.cedula_persona = fp.cedula_persona and v.id_calle = c.id_calle  and cc.id_comite = co.id_comite AND cc.cedula_persona = fp.cedula_persona ORDER BY `c`.`nombre_calle` ASC";
        $respuesta_arreglo = '';
        try {
            $datos = $this->conexion->prepare($tabla);
            $datos->execute();
     ...
     public function Carnet_Personas()
    {
        ...
        $tabla = "SELECT p.cedula_persona, p.primer_nombre, p.segundo_nombre, p.primer_apellido, p.segundo_apellido, c.nombre_calle, cp.* FROM familia f, familia_personas fp, personas p, vivienda v, calles c, carnets cp WHERE p.estado = 1  AND f.id_familia = fp.id_familia AND p.cedula_persona = fp.cedula_persona AND v.id_calle = c.id_calle and cp.cedula_persona = fp.cedula_persona ORDER BY `c`.`nombre_calle` ASC";
        $respuesta_arreglo = '';
        try {
            $datos = $this->conexion->prepare($tabla);
            $datos->execute();
    ...
    public function Discapacitados()
    {
        ...
        $tabla            = "SELECT DISTINCT dp.*, p.cedula_persona, d.nombre_discapacidad FROM personas p, discapacidad_persona dp, discapacidad d WHERE p.cedula_persona = dp.cedula_persona AND dp.id_discapacidad = d.id_discapacidad ORDER BY p.cedula_persona ASC";
        $respuesta_arreglo = '';
        try {
            $datos = $this->conexion->prepare($tabla);
            $datos->execute();
